...
class Node:
    def __init__(self, state: AlphaTrainableGame, parent=None):
        self.state = state
        self.parent = parent
        self.children = {}
        self.visits = 0
...
def mcts_search(root_state: AlphaTrainableGame, num_simulations: int):
...
def select_child(node: Node):
    # Use UCT (Upper Confidence Bound for Trees) to balance exploration and exploitation
...
def backpropagate(node: Node, results: AlphaGameResults):
...
def select_best_child(node: Node):
    best_child = None
